...
namespace Task3
{
    class Program
    {
        ...
        public static WireRoute CalculateRoute(string instructions)
        {
            var route = ParseList(instructions);

            WireRoute wireRoute = new WireRoute();
            foreach (var instruction in route)
            {
                wireRoute.Move(instruction.d, instruction.number);
            }

            return wireRoute;
        }

        public static List<(Direction d, int number)> ParseList(string multiInstructions)
        {
            return new List<(Direction d, int number)>(multiInstructions.Split(',').Select(Parse));
        }

        public static (Direction d, int number) Parse(string instruction)
        {
            return (GetDirection(instruction[0]), int.Parse(instruction.Substring(1)));
        }

        private static Direction GetDirection(char d)
        {
            return d switch
            {
                'R' => Direction.Right,
    ...
    {
        public C5.HashDictionary<Point, int> Points { get; }
        public C5.LinkedList<Point> Route { get; }

        public WireRoute()
        {
            Points = new HashDictionary<Point, int>();
            Route = new C5.LinkedList<Point>();
        }

        public void Move(Direction direction, int number)
        {
            if (number <= 0)
                return;

            var routeLast = Route.IsEmpty ? new Point(0,0) : Route.Last;
            var next = AddOneToDirection(direction, routeLast);
            Route.Add(next);
            if (!Points.Contains(next))
                Points.Add(next, Route.Count);

            Move(direction, --number);
        }

        private static Point AddOneToDirection(Direction direction, Point last)
        {
            switch (direction)
            {
                case Direction.Up:
